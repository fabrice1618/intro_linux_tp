...
- D Installation apache 2 et PHP 8
  ...
  - D4 Consultation des logs
- E Outils de développement
  - E1 Installation de Git sur le serveur LAMP
  - E2 Connexion à partir de VS Code
- F Préconisations pour sécuriser un serveur de production
  - F1 Sécurisation du système Linux
  - F2 Accès au serveur par SSH
  - F3 Sécurisation de MySQL
  - F4 Configuration de apache2 et de PHP
  - F5 Utilisation d'un IDS ? configuration du firewall ?
  - F6 Chiffrement SSL du serveur apache2
  - F7 Autres sécurisations possibles
...
- [APT - Gestion des paquets](apt.md)
- [UFW - Firewall](ufw.md)
- [Configuration Réseau](configuration_reseau.md)
- [Systemctl - Gestion des services](systemctl.md)
- [MySQL - Base de données](mysql.md)
- [Apache2 - Serveur Web et VirtualHosts](apache2.md)
- [PHP - Langage de script serveur](php.md)
...
### B1 Installation
Note: plusieurs distributions sont disponibles. par exemple debian. Il est aussi possible d'utiliser ubuntu desktop 24.04 si vous souhaitez tester les applications graphiques.
...
Remise à jour de tous les logiciels installés sur la distribution linux. Il est indispensable de remettre à jour votre distribution linux le plus souvent possible. Les mises à jour de sécurité sont disponibles très rapidement.
```
...
- Activation du firewall ufw et permission de l'accès SSH
```bash
$ ip a
$ sudo ufw enable
$ sudo ufw allow ssh
...
Activer tous les logs sur notre base de données. Cela nous permettra d'avoir un suivi de ce qu'il se passe sur le serveur. Ces options sont utiles sur des machines de développement. Notamment: cela permettra de détecter des erreurs de conceptions de l'application.

Activer les logs dans les cas suivants:
- le log principal dans /var/log/mysql/mysql.log
- le slow query log pour indiquer toutes les requêtes prenant plus de 2 secondes 
- les requêtes n'utilisant pas d'index. 

Nous autorisons aussi l'accès au serveur MySQL hors de la VM (à partir du guest). les paramètres bind-address permettent à des applications installées sur la machine host puissent accéder à la base de données. Accès possible aussi par le LAN.
...
Pour réaliser l'administration des bases de données, nous utilisons un user spécifique pour cette tâche, appelé dba par exemple (DataBase Administrator). Cet utilisateur doit avoir tous les accès sur toutes les bases de données et tous les privilèges. En général, les modifications de structure de la base de données sont faites avec cet utilisateur, en plus des sauvegardes et des restaurations.
...
Un autre utilisateur est créé pour se connecter à la base de données pour le compte de l'application. Pour améliorer la sécurité, on limite les privilèges de cet utilisateur. Cet utilisateur ne peut pas utiliser les autres bases de données présentes sur le serveur. On peut aussi limiter les requêtes SQL pouvant être exécutées (SELECT, INSERT, DELETE et UPDATE).

On se connecte à MySQL comme utilisateur _dba_ pour la création de la base de données et la création de l'utilisateur myapp.

> **Note :** En production, il est recommandé de limiter les privilèges de l'utilisateur applicatif aux seules opérations nécessaires (SELECT, INSERT, UPDATE, DELETE).
...
Test de la connexion de l'utilisateur myapp à la base de données myapp. Initialement, la base de données ne contient aucune table.
```
$ mysql -u myapp -p
Enter password:
Welcome to the MySQL monitor.  Commands end with ; or \g.
...
D'après les différences affichées: quelles configurations sont activées dans la version développement qui simplifient le développement ou son analyse de fonctionnement ?
- ......
- ......
...
En cas d'erreur, vous pouvez consulter les logs apache suivants. Les fichiers modifiés le plus récemment sont en bas.
